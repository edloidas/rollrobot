...
export const ru: Messages = {
  ...
  pick: {
    usage: 'Нужно хотя бы два варианта — /pick Камень | Ножницы | Бумага',
    tooMany: 'Слишком много вариантов — не больше 100.',
    spaceSplit: 'Разделено по пробелам — используй , или | для целых фраз.',
  },
...
/pick [варианты] — выбор наугад (сокращение: /p, бета)
/help — эта справка
...
<b>Выбор</b>
<code>/pick Камень | Ножницы | Бумага</code> — один вариант наугад
<code>/pick Аня, Боря, Вера</code> — запятые тоже работают
<code>/pick север юг восток запад</code> — отдельные слова можно без разделителя
<code>/pick Меч | Лук "Какое оружие?"</code> — назови выбор
Вставь список отдельными строками, чтобы вытянуть строку из таблицы.
...
Попробуй нотацию в ${playground('песочнице')} или открой ${reference('полный справочник по нотации')}.`,
  ...
  commands: [
    ...
    { command: 'pick', description: 'Выбор наугад — /pick Камень | Ножницы | Бумага' },
...
/roll даёт сумму, /full — разбор по кубикам, /ask — Yes или No, /pick — вариант наугад, /help — справку по нотации. Напиши @rollrobot в любом чате, чтобы бросить кубики без добавления бота.`,
};
